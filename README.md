...
## 📦 開発ロードマップ
1.  **フェーズ 1** ✅ 完了: 3Dアバターのブラウザ表示と基本的なリップシンクの実装
2.  **フェーズ 2** ✅ 完了: LLM連携による自然対話ロジックの実装（ヒアリング項目の管理）
3.  **フェーズ 3** ✅ 完了: 感情分析・表情制御・モーション制御・データ保存の実装
4.  **フェーズ 4** 🚀 次のフェーズ: ホスティング環境へのデプロイと最適化
...
インタビュー完了後、ブラウザの開発者ツール（F12 → Application → IndexedDB）で記録されたデータを確認できます：

```javascript
// ブラウザコンソールで実行
const db = await getDataStorage();
const records = await db.getAllRecords();
const stats = await db.getStats();
console.log('Interview Records:', records);
...
3. **モーション制御**
   - 骨格ベースのジェスチャー制御
   - 7 種類のモーション: うなずき、首振り、手振り、思考、肩すくめ、指差し、アイドル
   - 頭部・腕の自然な動き
   - イージング付きアニメーション

4. **データ保存・統計機能**
   - インタビュー記録の永続化
   - LocalStorage + IndexedDB 対応
   - 感情履歴の追跡
   - JSON/CSV エクスポート
   - インタビュー統計の生成
...
| コンポーネント | ファイル | 説明 |
|---|---|---|
| Emotion Analyzer | `lib/emotionAnalyzer.ts` | テキスト感情分析 |
| Expression Controller | `lib/expressionController.ts` | 表情制御 |
| Motion Controller | `lib/motionController.ts` | ジェスチャー制御 |
| Data Storage | `lib/dataStorage.ts` | データ永続化・統計 |
...
| 項目 | 制限 | 将来対応予定 |
|------|------|---------|
| **リップシンク** | 基本的な音量ベースのみ | 周波数解析による詳細制御 (Phase 4) |
| **マイク入力** | 日本語のみ対応 | 多言語対応予定 |
| **Web Speech API** | Firefox 非対応 | ポリフィル追加検討中 |
| **API キー保護** | クライアント側に露出 | バックエンド経由での呼び出しに移行予定 (Phase 4) |
| **オフライン対応** | なし（API が必須） | ローカルモデルの検討中 |
| **ブレンドシェイプ** | VRM 標準形状に依存 | カスタム形状対応の検討中 |

### ✅ Phase 3 で実装済み

| 機能 | 完了 |
|------|------|
| **テキスト感情分析** | ✅ ポジティブ/ネガティブ/ニュートラル自動判定 |
| **表情制御** | ✅ 5種類の表情アニメーション |
| **ジェスチャー制御** | ✅ 7種類のモーション |
| **データ保存** | ✅ LocalStorage + IndexedDB 対応 |
| **インタビュー統計** | ✅ JSON/CSV エクスポート機能 |
